...
| **预算调整**   | 双维度（总预算 + 年度）联动表单，收支平衡校验，可调额度锁定；追加下达兼任**年度预算编制**（余额锚定容量校验：目标年剩余计划 ≤ 项目剩余额度）；驳回后可编辑再提交，审批版本绑定防串版本 |
...
覆盖：编制审批、业务记录、双维度调整（含 §7.4/§7.5 可调额度、余额锚定容量护栏与安全护栏）、台账上卷、Excel 导入（标准模板 + 个人结算单）、统计（含经费余额口径/模糊匹配/非叶汇总）、附件（上传/打包路径构建/IDOR 防护）、审计快照、API Key（签发/撤销/过期/硬排除/scope 收窄/科目映射）等。
