...
- 重复防护([ADR 0002](./docs/adr/0002-duplicate-prevention.md)):所有写入口统一查重——单据编号与项目内未作废记录同号 = **硬重复**,服务端 409/422 拒绝、无强制通道;无编号行按指纹(年度+金额+日期+摘要)疑似提示,导入预览可人工强制。DB 部分唯一索引兜底;作废记录不占编号(撤销后可重导)。
