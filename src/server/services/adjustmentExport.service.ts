...
export async function exportAdjustmentDocx(
  ...
  const [project, subjects, annualBudgetRow, projectBudget] = await Promise.all([
  ...
  // 导出"原预算"须为审批前基线(§issue12):重建逻辑抽至 adjustmentBaseline.service,
  // 与审批详情(§issue15)共用;未生效单不扣减,已生效单按 approvedAt 参照。
  const baseline = await buildBaselineAmounts(adj);
  const annualCurrentBySubject = baseline.annual;
  const totalCurrentBySubject = baseline.total;
